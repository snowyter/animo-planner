...
- **Tokens.** The dark values for every token in `src/App.css` (`background`, `card`, `foreground`, `muted-foreground`, `border`, `primary`, elevation, the ambient wash, and the next-step strip's warn and quiet tones from ticket 05), with contrast checked to the same bar `docs/design-system.md` sets for light. Whether Archer Green `#15803d` needs a lighter dark-mode variant to hold 3:1 / 4.5:1.
