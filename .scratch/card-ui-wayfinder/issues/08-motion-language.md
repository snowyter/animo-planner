...
- **The inventory.** Rewrite `docs/design-system.md`'s motion inventory: for every surface in the ticket-04 direction — screen push/pop between plan list and workspace, the tool panel fold, tab/segment switch, sheets, popovers and menus, cards on hover/press, the next-step card's arrival and exit, solve results, the course board, the ghost handoff — what moves, which preset, CSS or `motion`.
