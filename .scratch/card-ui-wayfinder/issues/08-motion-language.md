...
- **The inventory.** Rewrite `docs/design-system.md`'s motion inventory: for every surface in the ticket-04 direction — screen push/pop between plan list and workspace, the tool panel fold, tab/segment switch, sheets, popovers and menus, cards on hover/press, the next-step card's arrival and exit (and, per ticket 05, its queue advancing to the next item and its shrink to one line), the sidebar collapsing and expanding (ticket 04) without animating the grid's layout, solve results, the course board, the ghost handoff — what moves, which preset, CSS or `motion`.
