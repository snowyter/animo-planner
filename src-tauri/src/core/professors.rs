//! Professor identity — the normalized key a preference is keyed on.
//!
//! A professor is the person named on a snapshot. The **professor key** is the
...
//! in the site's capitalisation never splits one professor into two.
//!
...
/// Produces the normalized professor key for a name, or `None` when the name
...
/// same key.
pub fn professor_key(name: &str) -> Option<String> {
...
mod tests {
    ...
    fn a_normal_name_produces_a_lowercased_key() {
        assert_eq!(professor_key("Bryant Lee"), Some("bryant lee".into()));
    }
    ...
    fn names_that_are_one_human_still_produce_different_keys() {
        ...
        let a = professor_key("Lee, Bryant");
        let b = professor_key("Bryant Lee");
        assert!(a.is_some());
        assert!(b.is_some());
