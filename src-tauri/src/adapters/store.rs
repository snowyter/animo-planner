...
/// Migration 3 (ticket 17): sections gain their captured term span, so an
...
mod tests {
    ...
    fn migration_is_idempotent_on_an_existing_database() {
        ...
        assert_eq!(version, 3, "all migrations apply exactly once");
    }
    ...
    fn migration_two_adds_the_is_sample_flag_to_plans() {
        ...
        let version: i64 = store
            ...
            .query_row("PRAGMA user_version", [], |row| row.get(0))
            .expect("user_version must be readable");
        assert_eq!(version, 3, "a fresh database runs every migration");
        assert!(
